"""
CNP EPIC-01 — P1-01 + P1-03 + P1-05
MQTT bridge with:
  - Fixed wildcard subscription (cnp/v1/nodes/+/# not +/+)
  - Injectable client factory for test isolation
  - MQTT rate limiting with per-client-ID caps and quarantine
  - Structured breach logging
"""
...
# ----------------------------------------------------------------
#  Rate limit constants — MQTT side (P1-05)
# ----------------------------------------------------------------
_MQTT_PER_CLIENT_MAX_PER_SEC   = 10
_MQTT_BURST_MAX_PER_5S         = 50
_MQTT_INVALID_THRESHOLD        = 3       # breaches per 60s → quarantine
_MQTT_QUARANTINE_SEC           = 300     # 5 minutes
_MQTT_BREACH_WINDOW_SEC        = 60
_MQTT_BURST_WINDOW_SEC         = 5


class _ClientRateState:
    """Per-client-ID sliding-window state."""
...
class GatewayMqttBridge:
    ...
    Dependency-injectable client factory enables full unit testing
    without a live broker (P1-03).

    Usage:
        # Production
        bridge = GatewayMqttBridge(db_path)

        # Tests
        bridge = GatewayMqttBridge(db_path, client_factory=mock_factory)
    """
    ...
    def __init__(
        ...
        self._ops_detector: Any = None  # injected by OpsServices

    def set_ops_detector(self, detector: Any) -> None:
        """Inject ops anomaly detector after bridge is started."""
        ...
        while True:
            try:
                async with self._client_factory() as client:
                    self.client = client
                    # P1-01 FIX: use # (multi-level wildcard) not +
                    # cnp/v1/nodes/+/+ missed cmd/out and config topics
    ...
    def _check_rate_limit(self, client_id: str, topic: str) -> bool:
        ...
        Records breach events on violation.
        """
    ...
    def _record_invalid(self, client_id: str, topic: str, reason: str) -> None:
        """Track invalid message and quarantine after threshold."""
        state = self._rate_states[client_id]
        now = time.monotonic()
    ...
    async def _ingest(
        ...
        # Route to handler by topic suffix
        ...
        dispatch: dict[str, Any] = {
            ...
            "cmd/in":         None,    # gateway publishes, not subscribes
            "config":         None,    # gateway publishes, not subscribes
    ...
    async def _handle_state(self, envelope: dict[str, Any]) -> None:
        # State updates refresh last_seen — reuse heartbeat path for now
        node_id = envelope.get("node_id")
        if node_id:
            import aiosqlite
...
def _extract_node_id(topic: str) -> str:
    """cnp/v1/nodes/{node_id}/... → node_id"""
    parts = topic.split("/")
    return parts[3] if len(parts) >= 4 else "unknown"


def _topic_suffix(topic: str) -> str:
    """cnp/v1/nodes/{node_id}/{suffix} → suffix (may contain /)"""
    parts = topic.split("/", 4)
