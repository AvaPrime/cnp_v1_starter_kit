"""
CNP EPIC-02 — P2-03 + P2-05
V1 HTTP compatibility adapter.

Mounted at /v1/compat in main.py.

Accepts V1 envelope format on all node-inbound paths:
  POST /v1/compat/node/hello
  POST /v1/compat/node/heartbeat
  POST /v1/compat/node/event
  POST /v1/compat/node/error
  POST /v1/compat/node/command_result
  GET  /v1/compat/node/commands/{node_id}

Field translations applied (per CNP-EXEC-002 §Phase 2):
  protocol         → protocol_version
  timestamp        → ts_utc
  payload.params   → payload.arguments  (commands)
  payload.event_id → message_id envelope field
  payload.error_code → payload.code + domain=LEGACY + severity=error
  battery: -1      → battery_pct: null
  capabilities.power_mode → capabilities.power.source

Every V1 key translation emits a DEPRECATION_V1_KEY structured log.
Same P1-07 auth middleware applies — no weaker auth for legacy nodes.
"""
...
from ..models.schemas import validate_bootstrap_token
from .routes import require_node_token   # reuse same auth dependency
...
# ----------------------------------------------------------------
#  Field translation map — V1 → V2
# ----------------------------------------------------------------

def _translate_envelope(raw: dict[str, Any]) -> dict[str, Any]:
    """
    Normalise a V1 message envelope to V2 canonical form.
    Returns the translated dict. Emits DEPRECATION_V1_KEY logs.
    """
    ...
    if "protocol" in raw and "protocol_version" not in raw:
        log.warning(
            ...
            "canonical_key=protocol_version node_id=%s", node_id
        )
    ...
    if "timestamp" in raw and "ts_utc" not in raw:
        log.warning(
            ...
            "canonical_key=ts_utc node_id=%s", node_id
        )
    ...
    # Ensure message_id — generate ULID-like if absent
    ...
    # Payload-level translations
    payload = raw.get("payload", {})

    # event_id → promote to envelope message_id + remove from payload
    if "event_id" in payload:
        log.warning(
            "DEPRECATION_V1_KEY original_key=payload.event_id "
            "canonical_key=message_id node_id=%s", node_id
        )
        raw["message_id"] = payload.pop("event_id")

    # params → arguments (command payloads)
    if "params" in payload and "arguments" not in payload:
        log.warning(
            "DEPRECATION_V1_KEY original_key=payload.params "
            "canonical_key=payload.arguments node_id=%s", node_id
        )
        payload["arguments"] = payload.pop("params")

    # error_code → code + domain + severity
    if "error_code" in payload:
        log.warning(
            "DEPRECATION_V1_KEY original_key=payload.error_code "
            "canonical_key=payload.code node_id=%s", node_id
        )
    ...
    if "power_mode" in caps:
        log.warning(
            ...
            "canonical_key=capabilities.power.source node_id=%s", node_id
        )
        power_mode = caps.pop("power_mode")
        caps.setdefault("power", {})["source"] = power_mode

    # Ensure requires_ack for events
...
async def compat_hello(
    ...
    log.info("compat.hello node_id=%s", node_id)
    ...
    # Return response in V1 format for legacy node compatibility
    return {
        "protocol": "CNPv1",
...
async def compat_command_result(
    ...
    # V1 command_result uses status values: executed | failed | rejected | queued
    # Map to V2 status field — already compatible
...
async def compat_get_commands(
    ...
    """
    Returns pending command in V1 format (params not arguments).
    """
    async with aiosqlite.connect(settings.gateway_db_path) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
    ...
    # Return in V1 envelope format (protocol not protocol_version, params not arguments)
